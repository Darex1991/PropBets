...
private
  ...
  def start_date_cannot_be_later_then_finish_date
    errors.add(:start_on, "can't be later then finish date") if start_on > finish_on
  end

  def finish_date_cannot_be_in_the_past
    errors.add(:finish_on, "can't be in the past") if !finish_on.blank? and finish_on < Date.today
  end
